...
/*
  ...
  Usage:
    node backend/scripts/clean-db.js                # delete all documents from known collections (asks to confirm)
    node backend/scripts/clean-db.js --force        # same, no prompt
    node backend/scripts/clean-db.js --drop         # DROP DATABASE (asks to confirm)
    node backend/scripts/clean-db.js --drop --force # DROP DATABASE without prompt
...
const { MONGODB_URI, MONGODB_DB, NODE_ENV } = require('../src/config/env');
...
async function main() {
  try {
    ...
    if (NODE_ENV !== 'production') {
      console.log('[INFO] NODE_ENV is not production. If you want to repopulate example data, restart the backend or run the migrate script.');
    }
  } catch (e) {
